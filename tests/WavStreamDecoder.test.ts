import {describe, expect, it, test, beforeAll, beforeEach} from "vitest";
import type {DecodedAudio, WavFormat} from "../src";
import {State, WavStreamDecoder} from "../src";
    ...
    test.each(Object.entries(fixtureProperties))(
        ...
        (fixtureName, expected) => {
            ...
            for (let i = 0; i < result.channelData.length; i++) {
                expect(result.channelData[i]?.length, `File: ${fixtureName} - channel[${i}].length`).toBe(expected.samplesPerChannel);
            }
        }
    );
